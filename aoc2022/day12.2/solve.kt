...
fun main() {
    ...
    // Brute force dijkstra to do all-sources shortest path.
    for (iRow in 0..grid.size - 1) {
        for (col in 0..grid[iRow].size - 1) {
            if (grid[iRow][col].elevation == 'a') {
                var path = edsger(Pair(iRow, col), target!!, grid)

                // Not technically correct but enough to figure out the solution for AoC.
                if (path.size > 1) {
                    print("${path.size - 1}\n")
                }
            }
