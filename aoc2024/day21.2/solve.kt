...
fun solve(code: String) =
    code.substring(0, code.length - 1).toLong() * shortestByPad(mutableMapOf(), 0, code.map { it })
...
fun shortestByPad(
    cache: MutableMap<Pair<Int, List<Char>>, Long>,
    iPad: Int,
    code: List<Char>,
): Long {
    val pads =
    ...
    for (key in code) {
        ...
        val scores = newInstructions.map { shortestByPad(cache, iPad + 1, it) }
