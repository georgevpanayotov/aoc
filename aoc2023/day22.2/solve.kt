...
fun main() {
    ...
    for (i in 0..<bricks.size) {
        ...
        queue.addLast(i)
        val willDrop = mutableSetOf<Int>()
        while (!queue.isEmpty()) {
            val iBrick = queue.removeFirst()
            // All of the bricks are only supported by iBrick  or ones that have fallen.
            val supported =
                findSupported(iBrick, bricks, map).filter {
                    // All of the bricks that haven't fallen or been disintegrated and are
                    // supporting `it`.
                    val supporting =
                        findSupporting(it, bricks, map)
                            .filter { it != iBrick && !willDrop.contains(it) }
                            .size

                    supporting == 0
                }

            willDrop.addAll(supported)
