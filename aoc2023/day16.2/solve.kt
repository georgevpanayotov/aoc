...
fun computeBeams(starting: Beam, grid: List<String>): Int {
    ...
    var currentBeams = mutableListOf(starting)
...
fun main() {
    ...
    val starting = mutableListOf<Beam>()
    for (i in 0..<grid.size) {
        starting.add(Beam(Pair(i, 0), Pair(0, 1)))
        starting.add(Beam(Pair(i, grid[0].length - 1), Pair(0, -1)))
    }

    for (i in 0..<grid[0].length) {
        starting.add(Beam(Pair(0, i), Pair(1, 0)))
        starting.add(Beam(Pair(grid.size - 1, i), Pair(-1, 0)))
    }

    val score = starting.map {
        computeBeams(it, grid)
    }.max()
